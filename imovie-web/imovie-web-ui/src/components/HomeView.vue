...
export default {
  ...
  data () {
    return {
      ...
      options: {
        ...
        editors: [],
        actors: [],
        categories: ['剧情', '爱情', '喜剧', '动作', '科幻', '奇幻', '冒险', '动画', '战争', '悬疑', '惊悚', '恐怖', '犯罪', '音乐', '歌舞', '历史', '传记', '家庭', '古装', '短片', '运动', '儿童', '西部', '武侠', '真人秀', '脱口秀', '纪录片'],
  ...
  created () {
    if (this.$auth.user.authenticated) {
      this.options.categories.push('情色')
      this.options.categories.push('同性')
    }
    this.loadData()
  },
  computed: {
